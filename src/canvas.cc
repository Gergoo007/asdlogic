...
// 	// dl->AddLine(ImVec2(center.x, 0), ImVec2(center.x, screen.y), 0xffffffff, 1 * maincanvas.zoom);
// 	// dl->AddLine(ImVec2(0, center.y), ImVec2(screen.x, center.y), 0xffffffff, 1 * maincanvas.zoom);

// 	const f32 GRID_SPACING_ZOOM = GRID_SPACING * zoom;
// 	// const f32 remainderx = ((center.x / GRID_SPACING_ZOOM) - truncf(center.x / GRID_SPACING_ZOOM)) * GRID_SPACING_ZOOM
// 	// 						+ fmodf(panpos.x * zoom, GRID_SPACING_ZOOM);
// 	// const f32 remaindery = ((center.y / GRID_SPACING_ZOOM) - truncf(center.y / GRID_SPACING_ZOOM)) * GRID_SPACING_ZOOM
// 	// 						+ fmodf(panpos.y * zoom, GRID_SPACING_ZOOM);

// 	const f32 remainderx = fmodf(panpos.x, GRID_SPACING_ZOOM);
// 	const f32 remaindery = fmodf(panpos.y, GRID_SPACING_ZOOM);

// 	for (f32 y = remaindery; y < screen.y; y += GRID_SPACING_ZOOM) {
// 		for (f32 x = remainderx; x < screen.x; x += GRID_SPACING_ZOOM) {
// 			dl->AddCircleFilled(ImVec2(x, y), 2 * zoom, 0xff3a3b3c);
// 		}
// 	}
...
void Canvas::drawGrid(ImDrawList* dl) {
	...
	// const f32 spacing = GRID_SPACING * zoom;

	// // Elmozgatja a grid-et hogy a pan simának tűnjön
	// const f32 panoffsetx = fmod(panpos.x, spacing);
	// const f32 panoffsety = fmod(panpos.y, spacing);

	// // Elmozgatja a gridet hogy úgy tűnjön mintha középre zoomolna az ember
	// const f32 zoomoffsety = 0;
	// const f32 zoomoffsetx = 0;

	// for (f32 y = panoffsety + zoomoffsety; y < screen.y; y += spacing) {
	// 	for (f32 x = panoffsetx + zoomoffsetx; x < screen.x; x += spacing) {
	// 		dl->AddCircleFilled(ImVec2(x, y), 2 * zoom, 0xff3a3b3c);
	// 	}
	// }
...
ImVec2 Canvas::convert(ImVec2 gridpos) {
	...
	// return gridpos * GRID_SPACING + center + panpos - ImVec2(gridoffsetx, gridoffsety)/2;
	return (gridpos + ImVec2(pancoordx, pancoordy)) * GRID_SPACING_ZOOM + center;
}

// void Canvas::drawGrid(ImDrawList* dl) {
// 	ImVec2 screen = ImVec2(1280, 720);
// 	ImVec2 center2 = (screen) / 2;

// 	dl->AddLine(ImVec2(center2.x, 0), ImVec2(center2.x, screen.y), 0xffffffff, 1 * zoom);
// 	dl->AddLine(ImVec2(0, center2.y), ImVec2(screen.x, center2.y), 0xffffffff, 1 * zoom);

// 	const f32 panoffsetx = fmod(panpos.x, GRID_SPACING);
// 	const f32 panoffsety = fmod(panpos.y, GRID_SPACING);

// 	for (f32 y = panoffsety; y < screen.y; y += GRID_SPACING) {
// 		for (f32 x = panoffsetx; x < screen.x; x += GRID_SPACING) {
// 			dl->AddCircleFilled(ImVec2(x, y), 2 * zoom, 0xff3a3b3c);
// 		}
// 	}
// }
...
// 	const f32 offsetx = fmod(center.x, GRID_SPACING);
...
// 	// offsetx az az hogy hányadik pixelnél kezdődik a grid

// 	return ImVec2(
// 		std::round((coord.x - panoffsetx) / GRID_SPACING) - pancoordx,
// 		std::round((coord.y - panoffsety) / GRID_SPACING) - pancoordy 
// 	);
// }
